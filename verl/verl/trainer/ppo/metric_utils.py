...
def compute_rep_distillation_metrics(batch: DataProto) -> dict[str, Any]:
    ...
    metrics: dict[str, Any] = {
        ...
        "rep/batch_size": teacher_repr.shape[0],
        "rep/positions": rep_positions,
        "rep/layers": rep_layers,
    }
    if rep_positions in ("first_k", "last_k"):
        metrics["rep/compact_token_len"] = teacher_repr.size(-2) if teacher_repr.dim() >= 3 else 1
    if multi_layer_rep:
