                    ...
                    with grad_ckpt_ctx:
                        ...
                        if use_rep_distillation:
                            ...
                            projector = self._get_or_create_rep_projector(
                                student_repr.size(-1), teacher_repr.size(-1)
                            )
                            ...
                            micro_batch_metrics["rep/norm_mse"] = rep_loss.detach().item()
                            micro_batch_metrics["rep/cosine_similarity"] = (
                                multi_layer_normalized_cosine_similarity(
                                    student_repr, teacher_repr, position_mask=position_mask, num_layers=num_rep_layers
                                )
                                .detach()
                                .item()
                            )
                            if num_rep_layers is not None:
                                micro_batch_metrics["rep/num_layers"] = num_rep_layers
